...
# 시퀀스를 NER 태그로 변환
def sequences_to_tag(sequences):    # 예측값을 index_to_ner로 태깅 정보로 변경하는 함수
    result = []
    for sequence in sequences:     # 전체 시퀀스로부터 시퀀스를 하나씩 꺼낸다.
        temp = []
        for pred in sequence:       # 시퀀스로부터 예측값을 하나씩 꺼낸다
            pred_index = np.argmax(pred)    # 예로 [0, 0, 0, 1, 0]이면 1의 인덱스
            temp.append(index_to_ner[pred_index].replace('PAD', '0'))   # PAD는 0으로 변경
        result.append(temp)
    return result
...
pred_tags = sequences_to_tag(y_predicted)   # 예측 NER
